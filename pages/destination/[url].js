...
const Destination = (response) => {
	...
	if (hotelList.length == 0) {
		...
		const fetcher  = axios.get(`${process.env.NEXT_PUBLIC_HOST_BE}/filter2?group_id=2533&city_name=${response.city}&star_rating=${starRating}&min_price=${minPrice}&max_price=${maxPrice}&amenities=${amenities.join()}`).then(response => {
